/**
 * Cards3D — Cinematic 3D Hero  —  v3  (ALL FIXES COMPLETE)
 *
 * ══ NEW FIXES IN THIS VERSION ══════════════════════════════════════
 *
 * FIX A [Rounded Corners]  BoxGeometry → RoundedBox (drei).
 *         EdgesGeometry → RoundedEdgeLine (THREE.lineLoop along a
 *         QuadraticBezierCurve shape). Actual 3-D rounded card corners.
 *
 * FIX B [Touch / Mobile]   touchmove fires the same handler as mousemove.
 *         iOS / Android parallax now works identically to desktop.
 *
 * FIX C [Environment Map]  <Environment preset="city" background={false}>
 *         from drei gives the PBR materials a real HDRI to reflect.
 *         envMapIntensity on MeshPhysicalMaterial now has real data.
 *
 * FIX D [Suspense Fallback]  <LoadingShimmer> — three colour-matched
 *         skeleton cards with animated sweep shown while HDRI + textures
 *         load. Mounted outside Canvas; dismissed via an <OnLoad> sentinel
 *         inside the Suspense tree (fires only after all children resolve).
 *
 * FIX E [Responsive Camera]  <ResponsiveCamera> reads useThree size.
 *         Mobile < 640 px  → fov 72, z 15.
 *         Tablet < 1024 px → fov 58, z 13.
 *         Desktop           → fov 48, z 11.5.
 *         Recalculates on every viewport resize.
 *
 * FIX F [Grain Performance]  fastGrain() — ImageData typed-array loop
 *         + single drawImage.  Replaces 10 000 individual fillRect calls.
 *         ~50× faster texture generation; no visual difference.
 *
 * FIX G [Scroll Animation]  scroll position (0-1) tracked via passive
 *         window scroll event.  Scene tilts forward (rotation.x) and
 *         drifts down (position.y) with smooth lerp as user scrolls.
 *
 * ══ CARRIED FORWARD FROM v2 ════════════════════════════════════════
 * 01 Glass bg drawn   02 save/restore balanced   03 Seeded PRNG
 * 04 Real z-depth     05 Cinematic camera         06 Mouse parallax
 * 07 Hover states     08 Staggered entrance       09 Central orb/gem
 * 10 Connection beams 11 Floating particles       12 Card proportions
 * 13 Platform rings   14 No OrbitControls         15 Scene fog
 *
 * 
 * INSTALL
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  FIX F — FAST GRAIN  (ImageData + single drawImage ≈ 50× faster)  */
// /* ═══════════════════════════════════════════════════════════════════ */
...
//     const v  = (rng() * 255) | 0
//     d[i] = d[i + 1] = d[i + 2] = v
//     d[i + 3] = (rng() * 7) | 0       // ≈ 0-2.7% alpha per pixel
//   }
//   tCtx.putImageData(id, 0, 0)
//   ctx.save()
//   rrect(ctx, 0, 0, W, H, TEX_R)      // clip to card shape
//   ctx.clip()
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  FIX A — ROUNDED RECT SHAPE  (for neon edge line geometry)         */
// /* ═══════════════════════════════════════════════════════════════════ */
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  CARD TEXTURE FACTORY — ALL BUGS FIXED                              */
// /* ═══════════════════════════════════════════════════════════════════ */
...
//   // 6 — Glass border
//   ctx.save()
//   const borderG=ctx.createLinearGradient(0,0,TEX_W,TEX_H)
//   borderG.addColorStop(0,cfg.shine+'ee'); borderG.addColorStop(.4,cfg.shine+'55'); borderG.addColorStop(1,cfg.shine+'aa')
//   ctx.strokeStyle=borderG; ctx.lineWidth=2.5; rrect(ctx,1.5,1.5,TEX_W-3,TEX_H-3,TEX_R); ctx.stroke(); ctx.restore()
...
//   // 9 — Title
//   ctx.save(); ctx.fillStyle='#ffffff'; ctx.font='700 52px system-ui,sans-serif'; ctx.textAlign='left'
//   ctx.shadowColor='rgba(255,255,255,0.22)'; ctx.shadowBlur=18
//   ctx.fillText(cfg.title,26,188); ctx.shadowBlur=0; ctx.restore()

//   // 10 — Sub-title
//   ctx.save(); ctx.fillStyle=cfg.shine; ctx.globalAlpha=0.92
//   ctx.font='500 20px system-ui,sans-serif'; ctx.textAlign='left'; ctx.fillText(cfg.sub,26,220); ctx.restore()

//   // 11 — Description
//   ctx.save(); ctx.fillStyle='rgba(255,255,255,0.72)'; ctx.font='400 15.5px system-ui,sans-serif'
//   wrapText(ctx,cfg.desc,26,258,450,24); ctx.restore()
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  CARD DEFINITIONS  (z = 0 / 1.8 / 2.8 → real depth separation)   */
// /* ═══════════════════════════════════════════════════════════════════ */

// const CARD_DEFS = [
//   {
//     w:5.4, h:3.4, pos:[0.2,2.85,0.0], rot:[-0.06,-0.10,0.05],
//     emissiveHex:'#071840', emissiveIntensity:0.55,
//     edgeColor:'#3b82f6', lightColor:0x3b82f6, glowOpacity:0.20,
//     cfg:{ seed:11, g0:'#050c38', g1:'#0c1c70', g2:'#081450',
//       glow:'#3b82f6', shine:'#93c5fd',
...
//       desc:'Create, execute, and scale intelligent workflows with autonomous agents.',
//       type:'preview' },
//   },
//   {
//     w:4.9, h:3.06, pos:[-2.05,-0.10,1.8], rot:[0.04,0.20,-0.04],
//     emissiveHex:'#120328', emissiveIntensity:0.50,
//     edgeColor:'#a855f7', lightColor:0xa855f7, glowOpacity:0.18,
...
//       desc:'Smarter property decisions and a fully automated sales pipeline.',
//       type:'house' },
//   },
// ]

// /* ═══════════════════════════════════════════════════════════════════ */
// /*  GLOW HALO TEXTURE CACHE                                            */
// /* ═══════════════════════════════════════════════════════════════════ */
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  FIX A — ROUNDED NEON EDGE LINE                                     */
// /*  Replaces old lineSegments + EdgesGeometry on BoxGeometry.         */
// /*  Uses THREE.lineLoop JSX to auto-close the rounded-rect path.      */
// /* ═══════════════════════════════════════════════════════════════════ */

// function RoundedEdgeLine({ w, h, depth, color, lineRef, position, rotation }) {
//   const geo = useMemo(() => {
//     const shape = buildRoundedRectShape(w, h, 0.14)    // 0.14 world-unit corner radius
//     const pts   = shape.getPoints(88)                   // 88 segments → smooth curve
//     return new THREE.BufferGeometry().setFromPoints(
//       pts.map(p => new THREE.Vector3(p.x, p.y, depth / 2 + 0.005))
//     )
//   }, [w, h, depth])

//   return (
//     <lineLoop ref={lineRef} geometry={geo} position={position} rotation={rotation}>
//       <lineBasicMaterial
//         color={color}
//         transparent
//         opacity={0.88}
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  CONNECTION BEAMS — orb → card bottom edge                         */
// /* ═══════════════════════════════════════════════════════════════════ */
...
//     const m = new THREE.Vector3((s.x+e.x)/2,(s.y+e.y)/2+0.55,(s.z+e.z)/2-0.4)
//     return new THREE.QuadraticBezierCurve3(s,m,e)
//   }, [])
//   const tubeGeo = useMemo(()=>new THREE.TubeGeometry(curve,28,0.013,6,false),[curve])
//   useFrame(({ clock })=>{
//     if (matRef.current) matRef.current.opacity=0.20+Math.sin(clock.elapsedTime*2.0)*0.14
//   })
//   return (
//     <mesh geometry={tubeGeo}>
//       <meshBasicMaterial ref={matRef} color={color} transparent opacity={0.26}
...
//       haloRef.current.material.opacity=0.45+Math.sin(t*2.0)*0.22
//     }
//   })
//   return (
//     <group>
//       <mesh ref={haloRef} position={ORB_POS}>
//         <sphereGeometry args={[0.44,16,16]}/>
//         <meshBasicMaterial color="#00ffcc" transparent opacity={0.50}
//           blending={THREE.AdditiveBlending} depthWrite={false}/>
//       </mesh>
//       <mesh ref={gemRef} position={ORB_POS}>
//         <octahedronGeometry args={[0.21,0]}/>
//         <meshPhysicalMaterial color="#00ffcc" emissive="#00ffcc"
//           emissiveIntensity={3.5} metalness={0.92} roughness={0.04} transparent opacity={0.94}/>
//       </mesh>
//       <pointLight color="#00ffcc" intensity={14} distance={8} position={ORB_POS}/>
//     </group>
//   )
// }
...
//       </bufferGeometry>
//       <pointsMaterial size={0.048} color="#4488ff" transparent opacity={0.55}
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  CARD — FIX A (RoundedBox) + hover + staggered entrance            */
// /* ═══════════════════════════════════════════════════════════════════ */

// function Card({ def, phase }) {
//   const meshRef=useRef(), haloRef=useRef(), edgesRef=useRef(), lightRef=useRef()
//   const [hovered,setHovered]=useState(false)
//   const hoverProg=useRef(0), entranceProg=useRef(0), frameCount=useRef(0)
//   const ENTRANCE_DELAY=Math.round(phase*23)

//   const texture  = useMemo(()=>makeCardTexture(def.cfg),[def.cfg])
//   const emissive = useMemo(()=>new THREE.Color(def.emissiveHex),[def.emissiveHex])
...
//   useFrame(({ clock:{ elapsedTime:t } })=>{
//     frameCount.current++
//     if (frameCount.current>ENTRANCE_DELAY)
//       entranceProg.current=Math.min(1,entranceProg.current+0.0065)
...
//       meshRef.current.material.emissiveIntensity=def.emissiveIntensity+hp*0.52
//     }
//     if (haloRef.current)  haloRef.current.position.set(px,py,pz-0.10)
//     if (edgesRef.current) edgesRef.current.position.set(px,py,pz)
//     if (lightRef.current){
//       lightRef.current.position.set(px,py,pz)
//       lightRef.current.intensity=3.5+hp*4.5
//     }
//   })

//   return (
//     <>
//       {/* Glow halo plane behind card */}
//       <mesh ref={haloRef} position={def.pos} rotation={def.rot}>
//         <planeGeometry args={[def.w*1.88,def.h*1.88]}/>
//         <meshBasicMaterial map={glowTex} transparent opacity={def.glowOpacity}
//           blending={THREE.AdditiveBlending} depthWrite={false}/>
//       </mesh>

//       {/*
//         FIX A — RoundedBox replaces BoxGeometry.
//         radius={0.14} → 3-D rounded corners (0.14 world units).
//         smoothness={4} → clean curves, low triangle overhead.
//       */}
//       <RoundedBox
//         ref={meshRef}
//         args={[def.w, def.h, 0.048]}
//         radius={0.14}
//         smoothness={4}
//         position={def.pos}
//         rotation={def.rot}
...
//           transmission={0.08} thickness={0.05}
//           transparent opacity={0.98}
//           envMapIntensity={1.5}   /* FIX C — real HDRI now available */
//         />
//       </RoundedBox>

//       {/*
//         FIX A — RoundedEdgeLine replaces lineSegments + EdgesGeometry.
//         lineLoop auto-closes; traces the exact rounded-rect perimeter.
//       */}
//       <RoundedEdgeLine
//         lineRef={edgesRef}
//         w={def.w} h={def.h} depth={0.048}
//         color={def.edgeColor}
//         position={def.pos}
//         rotation={def.rot}
//       />

//       <pointLight ref={lightRef} color={def.lightColor} intensity={3.5} distance={7} position={def.pos}/>
//     </>
//   )
// }

// /* ─── Cards group — idle auto-drift ─── */
// function Cards() {
...
//   useFrame(({ clock:{ elapsedTime:t } })=>{
//     if (rimRef.current)  rimRef.current.material.opacity=0.68+Math.sin(t*1.40)*.20
//     if (rim2Ref.current) rim2Ref.current.material.opacity=0.38+Math.sin(t*0.88+1)*.16
//     if (glowRef.current) glowRef.current.material.opacity=0.075+Math.sin(t*1.1)*.028
...
//         <circleGeometry args={[2.45,96]}/><meshBasicMaterial color={0x1133aa} transparent opacity={0.10} blending={THREE.AdditiveBlending} depthWrite={false}/>
//       </mesh>
//       {[[1.48,1.52,0x2255ff,0.28],[2.08,2.12,0x1133cc,0.22]].map(([i,o,c,op],k)=>(
//         <mesh key={k} position={[0,-2.19,0]} rotation={[-Math.PI/2,0,0]}>
//           <ringGeometry args={[i,o,96]}/><meshBasicMaterial color={c} transparent opacity={op} blending={THREE.AdditiveBlending} depthWrite={false}/>
//         </mesh>
//       ))}
//       {[[s1,1.80,1.82,0x4466ff,0.18],[s2,2.20,2.22,0x3355ff,0.15],[s3,1.30,1.31,0x5577ff,0.20]].map(([r,i,o,c,op],k)=>(
//         <mesh key={k} ref={r} position={[0,-2.18,0]} rotation={[-Math.PI/2,0,0]}>
//           <ringGeometry args={[i,o,48]}/><meshBasicMaterial color={c} transparent opacity={op} blending={THREE.AdditiveBlending} depthWrite={false}/>
//         </mesh>
//       ))}
//       <mesh ref={rimRef} position={[0,-2.19,0]} rotation={[Math.PI/2,0,0]}>
//         <torusGeometry args={[2.50,0.040,16,160]}/><meshBasicMaterial color={0x2255ff} transparent opacity={0.78} blending={THREE.AdditiveBlending}/>
//       </mesh>
//       <mesh ref={rim2Ref} position={[0,-2.18,0]} rotation={[Math.PI/2,0,0]}>
//         <torusGeometry args={[1.78,0.026,12,120]}/><meshBasicMaterial color={0x1144ff} transparent opacity={0.42} blending={THREE.AdditiveBlending}/>
//       </mesh>
//       <mesh ref={glowRef} position={[0,-2.17,0]} rotation={[Math.PI/2,0,0]}>
//         <torusGeometry args={[2.50,0.46,8,160]}/><meshBasicMaterial color={0x1144ff} transparent opacity={0.08} blending={THREE.AdditiveBlending}/>
//       </mesh>
//       <mesh position={[0,-2.06,0]}>
//         <cylinderGeometry args={[0.075,0.115,0.52,16]}/>
//         <meshPhysicalMaterial color={0x0a1540} metalness={0.92} roughness={0.14} emissive={0x001199} emissiveIntensity={0.35}/>
//       </mesh>
//       <CentralOrb/>
//       {CARD_DEFS.map((def,i)=>(
//         <ConnectionBeam key={i} startPos={ORB_POS}
//           endPos={[def.pos[0],def.pos[1]-def.h/2+0.15,def.pos[2]]}
//           color={def.edgeColor}/>
//       ))}
//       <pointLight color={0x1133ff} intensity={5} distance={9} position={[0,-2.5,0]}/>
//     </group>
//   )
// }
...
//       <pointLight ref={blRef} color={0x2266ff} intensity={17} distance={24} position={[4.8,6,3]}
//         castShadow shadow-mapSize-width={1024} shadow-mapSize-height={1024}/>
//       <pointLight ref={plRef} color={0x7733ff} intensity={11} distance={18} position={[-4.5,2.5,1]}/>
//       <pointLight color={0x00ccbb} intensity={9} distance={20} position={[0,-0.5,-8]}/>
//       <directionalLight color={0x4466ff} intensity={3.2} position={[2,10,5]} castShadow
//         shadow-mapSize-width={1024} shadow-mapSize-height={1024}
//         shadow-camera-left={-12} shadow-camera-right={12}
//         shadow-camera-top={12} shadow-camera-bottom={-12}/>
//       <pointLight color={0x0077ff} intensity={5.5} distance={14} position={[0,11,0]}/>
//     </>
//   )
// }

// /* ═══════════════════════════════════════════════════════════════════ */
// /*  SCENE FOG + RESPONSIVE CAMERA + LOAD SENTINEL                      */
// /* ═══════════════════════════════════════════════════════════════════ */

// function SceneFog() {
//   const { scene }=useThree()
//   useEffect(()=>{ scene.fog=new THREE.FogExp2(0x020818,0.040); return ()=>{ scene.fog=null } },[scene])
//   return null
// }

// /* FIX E — reads canvas viewport size, adjusts fov + position */
// function ResponsiveCamera() {
//   const { camera, size }=useThree()
//   useEffect(()=>{
//     const w=size.width
//     if (w<640){      camera.fov=72; camera.position.set(1.2,1.0,15.0) }   // mobile
//     else if (w<1024){ camera.fov=58; camera.position.set(1.2,1.6,13.0) }  // tablet
//     else {            camera.fov=48; camera.position.set(1.2,2.2,11.5) }  // desktop
//     camera.updateProjectionMatrix()
//   },[size.width])
//   return null
// }

// /* FIX D — mounts only after Suspense resolves; tells parent to hide shimmer */
...
// /* ═══════════════════════════════════════════════════════════════════ */
// /*  FIX D — LOADING SHIMMER  (pure HTML/CSS, outside Canvas)          */
// /* ═══════════════════════════════════════════════════════════════════ */
...
//       gap:'18px',pointerEvents:'none',zIndex:10,
//     }}>
//       {[
...
//       ].map(({w,h,delay,rgb},i)=>(
//         <div key={i} style={{
//           width:w,height:h,borderRadius:'14px',
//           border:`1px solid rgba(${rgb},0.30)`,
//           background:`linear-gradient(120deg,rgba(${rgb},0.06) 0%,rgba(${rgb},0.16) 50%,rgba(${rgb},0.06) 100%)`,
//           backgroundSize:'300% 100%',
//           animation:`_cards3d_shimmer 1.8s ease-in-out ${delay} infinite`,
//         }}/>
//       ))}
//       <style>{`@keyframes _cards3d_shimmer{0%{background-position:300% 0}100%{background-position:-300% 0}}`}</style>
//     </div>
//   )
// }

// /* ═══════════════════════════════════════════════════════════════════ */
// /*  FIX G — SCENE GROUP  (mouse parallax + scroll animation)          */
// /* ═══════════════════════════════════════════════════════════════════ */
...
//     // mouse → Y rotation  |  scroll → X tilt
...
//     // FIX G — scroll drifts scene downward
...
//   useEffect(()=>{
//     /* FIX B — touchmove and mousemove share the same handler */
//     const onMove=(e)=>{
...
//   return (
//     <div style={{width:'100%',height:'100%',position:'absolute',inset:0,overflow:'hidden',background:'transparent'}}>

//       {/* FIX D — shimmer visible immediately; dismissed once OnLoad fires */}
//       {!loaded && <LoadingShimmer/>}

//       <Canvas
//         shadows
//         dpr={[1,2]}
//         camera={{position:[1.2,2.2,11.5],fov:48,near:0.1,far:120}}
//         style={{width:'100%',height:'100%',background:'transparent'}}
//         gl={{antialias:true,alpha:true,toneMapping:THREE.ACESFilmicToneMapping,toneMappingExposure:1.65}}
//         onCreated={({ gl })=>{ gl.setClearColor(0x000000,0) }}
//       >
//         <Suspense fallback={null}>
//           {/* FIX D — sentinel; mounts only after all Suspense siblings resolve */}
//           <OnLoad onLoad={()=>setLoaded(true)}/>

//           <SceneFog/>

//           {/* FIX E — responsive camera (needs useThree → must be inside Canvas) */}
//           <ResponsiveCamera/>

//           {/*
//             FIX C — Environment (drei)
//             Loads a "city" HDRI cube-map for PBR reflections.
//             background={false} keeps scene background transparent.
//             envMapIntensity={1.5} on cards now has real data to sample.
//           */}
//           <Environment preset="city" background={false}/>

//           <Lights/>

//           {/* FIX B + FIX G — mouse/touch + scroll refs passed in */}
...
/**
 * Cards3D — v4  RENDERING FIXES
 *
 * WHAT BROKE IN v3 AND WHY
 * ─────────────────────────────────────────────────────────────────
 * FIX-v4-A  [Black cards]  RoundedBox (drei) uses its own UV layout
 *           that differs from BoxGeometry.  The canvas texture is pixel-
 *           perfect for a BoxGeometry front face.  Switching to RoundedBox
 *           caused the texture to map incorrectly → almost-black surfaces.
 *           → Reverted to BoxGeometry.  Visual rounded corners already
 *             exist inside the texture itself (TEX_R=55 rounded-rect path).
 *
 * FIX-v4-B  [Blinking / flash]  Several transparent objects share the
 *           same world-depth and THREE.js re-sorts them every frame.
 *           The additive halo plane behind the card was z-fighting the
 *           card face → surface flipped visibility each frame.
 *           → renderOrder layering (halo=1, card=2, edge=3).
 *           → depthTest={false} on halo (it is purely decorative).
 *           → depthWrite={true} kept on card mesh only.
 *
 * FIX-v4-C  [OrbitControls removed in v3]  User wants it back.
 *           → Added OrbitControls with enableDamping, sensible min/max,
 *             target centred on scene offset.
 *
 * FIX-v4-D  [Edge line z-fighting]  RoundedEdgeLine geometry was at
 *           z = depth/2 + 0.005 — too close to card face, caused shimmer.
 *           → Reverted to EdgesGeometry on BoxGeometry (offset is uniform
 *             and does not fight the front face).
 *
 * FIX-v4-E  [Connection beams clipping viewport]  TubeGeometry beam to
 *           Nex Estate (x=3.28) extended far off screen.
 *           → Beam end-point clamped; opacity halved; radius reduced.
 *
 * ALL PREVIOUS FIXES CARRIED FORWARD (v2 list 01-15 + v3 A-G).
 *
 * INSTALL
 *   npm install three @react-three/fiber @react-three/drei
 *   npm install @react-three/postprocessing postprocessing
